...
class EventProcessor extends EventEmitter {
    ...
    setupHandlers() {
        this.on('ingested_event', async (event) => {
            const storedEvent = await eventRepositorio.storeEvent(event);
    ...
    async processEvent(event) {
        const { id, type, source, payload } = event;

        try {
            // 1. Marca o evento como 'em processamento'
            await eventRepositorio.updateEventStatus(id, 'processing');
            ...
            await eventRepositorio.updateEventStatus(id, 'completed');
            this.processedCount++;

        } catch (error) {
            console.error(`[PROCESSOR ERR] Failed to process ${id}:`, error.message);
            // 4. Marca como falho em caso de erro
            await eventRepositorio.updateEventStatus(id, 'failed', error.message);
        }
    }
