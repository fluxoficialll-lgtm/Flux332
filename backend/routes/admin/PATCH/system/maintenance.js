...
export default async (req, res) => {
    try {
        ...
        const currentRes = await CentralizadorDeGerenciadoresDeDados.query("SELECT value FROM platform_settings WHERE key = 'system_config'");
        let config = currentRes.rows[0]?.value || {};
        config.maintenanceMode = !!enabled;
        if (message) config.maintenanceMessage = message;

        await CentralizadorDeGerenciadoresDeDados.query(
